"""
Delta noScale 모델 전용 평가 스크립트.
- 역 스케일링 하드코딩 OFF (noScale 학습 모델과 쌍)
- 기본 경로: noScale 데이터 / noScale LoRA 모델
- 기존 01_test2_delta_evalation.py 와 동일 구조, scaling 고정 비활성화
"""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Delta noScale model evaluation on test JSONL.")
    parser.add_argument("--base-model-dir", default="../Meta-Llama-3.1-8B_tokenizerExtension")
    parser.add_argument("--lora-model-dir", default="../BaseLine/noScale_delta_fileLevel_lora")
    parser.add_argument(
        "--test-jsonl",
        default="../BaseLine/스케일링 미적용 데이터/finetune_dataset_delta_noScale_fileLevel_test.jsonl",
    )
    ...
    parser.add_argument("--output-report", default="./delta_noScale_eval_report.json")
    parser.add_argument(
...
def main() -> None:
    ...
    print(f"[INFO] delta scaling: DISABLED (noScale model)")
    ...
    for idx, row in enumerate(samples, 1):
        ...
        print(f"[{idx}/{total}] action={action} mode={mode_label} parse={'ok' if pred_deltas else 'fail'} aligned_points={m['aligned_points']}")
    ...
    if accumulate_to_absolute:
        print(f"  [NOTE] Metrics in ABSOLUTE space (delta accumulated from frame {obs_end_index})")
    else:
        print(f"  [NOTE] Metrics in DELTA space (no inverse scaling)")
