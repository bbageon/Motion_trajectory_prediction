...
lora_model_dir = "./BaseLine/motionQA_delta_finetuned_lora_mps"
...
# 2️⃣ 프롬프트 생성
# ----------------------------
pose_json_path = "./collected_motions/raise_arm_vertical.json"
...
# ... (6번 섹션 끝난 후, 7번 섹션 시작 전이나 직후에 추가)

# 🔥 [수정] max_frames 계산 코드 추가
if pred_deltas:
    # 첫 번째 관절의 예측 길이를 기준으로 설정
    max_frames = len(next(iter(pred_deltas.values())))
else:
    max_frames = 0

print(f"📊 Predicted Frames: {max_frames}")
...
# baseline에 존재하는 관절만 사용
abs_positions = {
    j: accumulate(baseline[j], seq) for j, seq in pred_deltas.items() if j in baseline
}
...
if max_frames > 0:
    ...
    print("❌ No valid predicted delta coordinates found. Check your parsing or accumulation logic.")