"""
Absolute ×100 모델 전용 평가 스크립트.
- 학습 시 coord × 100으로 토큰화된 모델 전용
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Absolute x100 model evaluation on test JSONL.")
    parser.add_argument("--base-model-dir", default="../Meta-Llama-3.1-8B_tokenizerExtension")
    parser.add_argument("--lora-model-dir", default="../BaseLine/x100_absolute_fileLevel_lora")
    parser.add_argument(
        "--test-jsonl",
        default="../BaseLine/x100 데이터/finetune_dataset_absolute_x100_fileLevel_test.jsonl",
    )
    ...
    parser.add_argument("--output-report", default="./absolute_x100_eval_report.json")
    parser.add_argument(
...
def _parse_numeric_fragment(fragment: str) -> float | None:
    ...
    if len(digits) >= 8:
        core = digits[-8:]
        raw = sign * (int(core[:3]) + int(core[3:8]) / DEC_SCALE)
        return round(raw / X100_FACTOR, DEC_DIGITS + 1)
    m = re.search(r"[-+]?\d+(?:\.\d+)?", frag)
    if m:
        try:
            return round(float(m.group(0)) / X100_FACTOR, DEC_DIGITS + 1)
        except ValueError:
            return None
    return None
...
def main() -> None:
    ...
    print(f"  [NOTE] Metrics in original coordinate space (÷{X100_FACTOR} applied)")
